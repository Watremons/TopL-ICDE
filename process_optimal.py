...
def is_pass_pruning_entry_optimal(entry: dict, radius: int, query_bv: int, query_support: int):
    # {
    #     "P": index_node,
    #     "R": [{
    #         "BV_r": 0,
    ...
    while len(max_heap_H) > 0:
        ...
        for child_entry in now_entry.index_N['P']:
            if child_entry['T']:  # N is a leaf node with child entry made by vertex
                ...
                if is_pass_pruning_entry_optimal(entry=child_entry, radius=radius_r_idx, query_bv=q_bv, query_support=query_support_k-2):  # check the community-level pruning
